...
public fun <T> (suspend () -> T).startCoroutineAtomic(completion: Continuation<T>) =
    isolateCoroutine(completion) { createCoroutineUnintercepted(it) }.intercepted().resume(Unit)
...
public fun <R, T> (suspend (R) -> T).startCoroutineAtomic(receiver: R, completion: Continuation<T>) =
    isolateCoroutine(completion) { createCoroutineUnintercepted(receiver, it) }.intercepted().resume(Unit)
